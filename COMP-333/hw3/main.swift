...
// Parser<Exp> = (MyList<Token>) -> ParseResult<Exp>
func expressionP() -> Parser<Exp> { // DO NOT MODIFY THIS LINE
	...
	let p_int = mapP(parser: numP(), operation: { num in Exp.intLiteral(num) })
	let p_true = oneToOneP(tokenRep: Token.trueToken, expRep: Exp.trueExp)
	let p_false = oneToOneP(tokenRep: Token.falseToken, expRep: Exp.falseExp)
	let p_negate = unopP(tokenRep: Token.minus, makeOp: { exp in Exp.negate(exp) })
	let p_not = unopP(tokenRep: Token.notToken, makeOp: { exp in Exp.not(exp) })
	let p_plus = binopP(tokenRep: Token.plus, binopRep: Binop.plus)
	let p_minus = binopP(tokenRep: Token.minus, binopRep: Binop.minus)
	let p_mult = binopP(tokenRep: Token.mult, binopRep: Binop.mult)
	let p_div = binopP(tokenRep: Token.div, binopRep: Binop.div)
	let p_and = binopP(tokenRep: Token.andToken, binopRep: Binop.andBinop)
	let p_or = binopP(tokenRep: Token.orToken, binopRep: Binop.orBinop)

	return orP(p_int, orP(p_true, orP(p_false, orP(p_negate, orP(p_not,
			orP(p_plus, orP(p_minus, orP(p_mult, orP(p_div, orP(p_and, p_or))))))))))
