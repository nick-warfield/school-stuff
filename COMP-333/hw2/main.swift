...
indirect enum MyList<A> {
	...
	func append(other: MyList<A>) -> MyList<A> {
		...
		case let .cons(value, tail):
			return MyList<A>.cons(value, tail.append(other: other))
		}
	}
	...
	func length() -> Int {
		switch self {
		case .empty: return 0
		case .cons(_, let tail): return 1 + tail.length()
		}
	}
	...
	func foldLeft<B>(accum: B, fold: (B, A) -> (B) ) -> B {
		...
		case let .cons(a, tail):
			let b = fold(accum, a)
			return tail.foldLeft(accum: b, fold: fold)
		}
	}
	func foldRight<B>(accum: B, fold: (A, B) -> (B) ) -> B {
		...
		case .empty: return accum
		case let .cons(a, tail):
			let b = tail.foldRight(accum: accum, fold: fold)
			return fold(a, b)
		}
	}
	...
	func filter(predicate: (A) -> Bool) -> MyList<A> {
		switch self {
		case .empty: return MyList<A>.empty
		case let .cons(a, tail):
			return predicate(a) ? MyList.cons(a, tail.filter(predicate: predicate)) :
								 tail.filter(predicate: predicate)
		}
	}
	...
	func contains(target: A, compare: (A, A) -> Bool) -> Bool {
		...
		case let .cons(a, tail): return compare(a, target) ? true :
			tail.contains(target: target, compare: compare)
		}
	}

	func sum(zero: A, add: (A, A) -> A) -> A {
		return self.foldLeft(accum: zero, fold: add)
	}
}
