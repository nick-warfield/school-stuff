...
sum41(Sum, Num, Num, Sum).
sum41(Accum, Lo, Hi, Sum) :-
	Lo < Hi,
	NewLo is Lo + 1,
	NewAccum is NewLo + Accum,
	sum41(NewAccum, NewLo, Hi, Sum).
...
% I can put the prereqs into a list, then put there prereqs into the list
% keep going until I can no longer add a prereq (fail), or the prereq is found (pass)
