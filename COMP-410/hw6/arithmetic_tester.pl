...
makeTestWithNums([Head|Tail], X) :-
	X = number(Head);
	makeTestWithNums(Tail, X).

makeTestWithNums(Depth, Nums, X) :-
	makeTestWithNums(Nums, X);
	dec(Depth, Next),
	makeTestWithNums(Next, Nums, A),
	makeTestWithNums(Next, Nums, B),
	(X = plus(A, B);
		X = minus(A, B);
		X = mult(A, B));
	dec(Depth, Next),
